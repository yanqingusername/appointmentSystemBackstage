    ...
    >
      ...
      <el-form-item label="所在城区">
        <el-select
          filterable
          clearable
          v-model="searchObj.samplingPlace"
          placeholder="请选择所在区"
        >
          <el-option
      ...
      <el-form-item label="站长" prop="samplingLeader">
        <el-input
          v-model="searchObj.samplingLeader"
          placeholder="请输入站长姓名"
        />
      </el-form-item>
      <el-form-item label="小程序是否上线">
        <el-select
          v-model="searchObj.fixedSamplingPoint"
          filterable
          clearable
          placeholder="请选择上线状态"
        >
        ...
        >
          ...
          >
            ...
            <!-- </div> -->
              ...
              <el-form-item label="详细地址" placeholder="请输入详细地址"  prop="siteDescription">
                <el-input
                  v-model="LisChannel.siteDescription"
                  placeholder="请输入详细地址"
                ></el-input>
            ...
            <el-form-item label="营业时间" label-width="180px">
              <div
            ...
            <div>
              <el-form-item label="运营标签">
                <el-checkbox-group v-model="LisChannel.tag">
                  <el-checkbox label="24小时营业" type="name"></el-checkbox>
                  <el-checkbox label="机场火车站" type="name"></el-checkbox>
                  <el-checkbox label="排队人少" type="name"></el-checkbox>
                  <el-checkbox label="停车方便" type="name"></el-checkbox>
                  <el-checkbox label="靠近商圈" type="name"></el-checkbox>
                  <el-checkbox label="无午休" type="name"></el-checkbox>
                </el-checkbox-group>
              </el-form-item>
            </div>
    ...
    <el-table :data="list" stripe style="width: 100%">
      ...
      >
        <template slot-scope="scope">
          <p v-if="scope.row.pointnature == '0'">自营</p>
          <p v-if="scope.row.pointnature == '1'">合作</p>
          <p v-if="scope.row.pointnature == '2'">第三方</p>
        </template>
      </el-table-column>
      <el-table-column
        prop="samplingPlace"
      ...
      <el-table-column
        ...
        align="center"
      />
      <el-table-column width="138" label="营业时间" align="center">
        <template slot-scope="scope">
          <p>{{ scope.row.sbusinessTime }}</p>
          <p>{{ scope.row.xbusinessTime }}</p>
        </template>
      </el-table-column>
      <el-table-column label="是否可用" width="80" align="center">
        <template slot-scope="scope">
          {{ scope.row.isOpen == 1 ? "可用" : "已停用" }}
        </template>
      </el-table-column>
      <el-table-column label="小程序上线" width="100" align="center">
        <template slot-scope="scope">
          {{ scope.row.fixedSamplingPoint == 1 ? "是" : "否" }}
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    ...
    var validatePass1 = (rule, value, callback) => {
      if (this.LisChannel.pointnature === "0" && value === "") {
          console.log("55555555551111111111555555")
        callback(new Error("请输入纬度"));
      } 
      else if(this.LisChannel.fixedSamplingPoint =='1' && this.LisChannel.latitude ==''){
        console.log("1111111111111111111555555")
        callback(new Error("请输入纬度"));
      } 
      else{
        console.log("5555588888811111111555555")
        callback()
      }
    };
    var validatePass2 = (rule, value, callback) => {
      ...
      else if(this.LisChannel.fixedSamplingPoint =='1' && this.LisChannel.longitude =='') {
        callback(new Error("请输入经度"));
      } 
      else{
        callback()
    ...
    return {
      ...
      LisChannel: {
        ...
        tag: [],
        paytype: ['默认'],
        sourceInfo: [14, 142, 136, 82, 74, 134],
        fixedSamplingPoint: "",
        supportCoyoteWxPay: "",
        zfbSamplingPoint: "",
        businessLeader: "",
      ...
      rules: {
        ...
        isBigScreen: [
          { required: true, message: "请选择是否大筛", trigger: "change" },
        ],
        ...
        longitude: [{ validator: validatePass2, trigger: "blur" }],
        latitude: [{ validator: validatePass1, trigger: "blur" }],
        appointmentOpen: [
        ...
        time_start: [{ validator: validatePass5, trigger: "blur" }],
        time_end: [
          { required: true, message: "请输入结束时间", trigger: "blur" },
        ],
        appointmentSum: [
  ...
  methods: {
    ...
    getList(page = 1) {
      ...
      point
        .getPointList(this.current, this.limit, this.searchObj)
        .then((response) => {
    ...
    submitForm(formName) {
      ...
      console.log(this.LisChannel.fixedSamplingPoint =='1' && this.LisChannel.latitude =='');
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (
            this.LisChannel.pointnature == "0" &&
            this.arrayData[0].time_start !== ""
          ) {
            console.log("执行1");
            this.resolveData();
            this.save();
    ...
    resolveData() {
      ...
      if (this.LisChannel.appointmentOpen == 1) {
        this.formData.appointmentTime =
          this.LisChannel.classes_time_start +":00"+
          "-" +
          this.LisChannel.classes_time_end+":00";
    ...
    save() {
      ...
      this.formData.samplingPlace = this.LisChannel.appointmentOpen;
      this.formData.createName = this.name;
